...
public class HideOrShowViewController: UIViewController {
    ...
    private func updateTableViewInset() {
        let tabBarVC = self.tabBarController as? TFYSwiftTabbarController
        let isHide = tabBarVC?.tabBar.isHidden ?? false
        if isHide {
            self.tableView.contentInset = UIEdgeInsets(top: UIDevice.tfy.statusBarHeight + 44.0, left: 0, bottom: UIDevice.tfy.homeIndicatorHeight, right: 0)
        } else {
            self.tableView.contentInset = UIEdgeInsets(top: UIDevice.tfy.statusBarHeight + 44.0, left: 0, bottom: self.tabBarController?.tabBar.frame.height ?? 0, right: 0)
        }
    }
}
